...
const fetcher = async (): Promise<Row[]> => {
  const { data, error } = await supabase
    ...
    .select('id, patient_id, created_at, studies, notes, folio, doctor_name, doctor_license, patients(first_name,last_name)')
...
export default function RadiologyOrdersPage() {
  const q = useSWR('radiology_orders', fetcher)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <h1 className="page-title">Solicitudes de Radiología</h1>
        ...
        <table className="w-full border text-sm">
          <thead>
            <tr className="bg-neutral-100">
              ...
              <th className="border px-2 py-1 text-left">Estudios</th>
          ...
          <tbody>
            {rows.map((r, i) => {
              ...
              const folioVisible = fmt(total - i, 'RX-')
              const studiesText =
                Array.isArray(r.studies) && r.studies.length
                  ? r.studies.map((s: any) => (typeof s === 'string' ? s : s?.name ?? '—')).join(', ')
                  : '—'
              return (
                <tr key={r.id}>
                  ...
                  <td className="border px-2 py-1">{studiesText}</td>
                  <td className="border px-2 py-1 text-center">
                    <Link href={`/radiology-orders/${r.id}`} className="text-emerald-700 hover:underline">Abrir</Link>
                  </td>
                </tr>
              )
            })}
