    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3 flex-wrap">
        <div>
          ...
          {!loading && <p className="text-xs text-gray-400 mt-0.5">{total} receta{total !== 1 ? 's' : ''} · {groups.length} paciente{groups.length !== 1 ? 's' : ''}</p>}
        </div>
        <Link href="/prescriptions/new" className="btn ml-auto text-sm">+ Nueva receta</Link>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        <div className="card p-12 text-center">
          ...
          <div className="font-semibold text-gray-500">{search ? 'Sin resultados' : 'Sin recetas médicas'}</div>
        </div>
      )}
          ...
          <div key={g.patient_id ?? 'sin'} className="card overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-3.5 bg-gray-50/60 border-b border-gray-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-400">{g.rows.length} receta{g.rows.length !== 1 ? 's' : ''}</div>
              </div>
            </div>
            <div className="divide-y divide-gray-50">
              {g.rows.map(r => (
                <div key={r.id} className="flex items-center gap-4 px-5 py-3 group hover:bg-gray-50/60 transition">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-gray-800">{r.folio ?? `REC-${r.id.slice(0, 8)}`}</div>
