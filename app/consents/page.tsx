  ...
  useEffect(() => {
    (async () => {
      ...
      (data ?? []).forEach((r:any) => {
        ...
        const row: Row = { id: r.id, dt: r.signed_at, folio: r.folio_code ?? null };
            ...
            <div className="overflow-x-auto">
              <table className="w-full border text-sm">
                ...
                <tbody>
                  {g.rows.map(r => (
                    <tr key={r.id}>
                      <td className="border px-2 py-1">{r.folio ?? ('CON-' + r.id.slice(0,8))}</td>
                      <td className="border px-2 py-1">{fmt.format(new Date(r.dt))}</td>
                      <td className="border px-2 py-1"><Link href={`/consents/${r.id}/print`} className="text-blue-600">Imprimir</Link></td>
                    </tr>
                  ))}
