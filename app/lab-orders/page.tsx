  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <Link href="/lab_orders/new" className="btn ml-auto">Nueva</Link>
      </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border text-sm">
                ...
                <tbody>
                  {g.rows.map(r => (
                    <tr key={r.id}>
                      <td className="border px-2 py-1">{r.folio ?? ('LAB-' + r.id.slice(0,8))}</td>
                      <td className="border px-2 py-1">{fmt.format(new Date(r.dt))}</td>
                      <td className="border px-2 py-1"><Link href={`/lab_orders/${r.id}/print`} className="text-blue-600">Imprimir</Link></td>
                    </tr>
                  ))}
