...
export default function LabPage() {
  ...
  async function deleteOrder(id: string) {
    if (!confirm('¿Eliminar esta solicitud de laboratorio?')) return
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Link href="/lab-orders/new" className="btn ml-auto text-sm">+ Nueva solicitud</Link>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        <div className="card p-12 text-center">
          ...
          <div className="font-semibold text-gray-500">{search ? 'Sin resultados' : 'Sin solicitudes de laboratorio'}</div>
        </div>
      )}
            ...
            <div className="divide-y divide-gray-50">
              {g.rows.map(r => (
                <div key={r.id} className="flex items-center gap-4 px-5 py-3 group hover:bg-gray-50/60 transition">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-gray-800">{r.folio ?? `LAB-${r.id.slice(0, 8)}`}</div>
                  ...
                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition shrink-0">
                    <Link href={`/lab-orders/${r.id}/print`}
